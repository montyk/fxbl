...
class Login extends MY_Controller {
    ...
    public function send_password() {
        ...
        if ($pwd_details) {
            ...
            $email_data['subject'] = $this->config->item('project_name').'Login Details';
    ...
    public function resend_verification_link($email = '') {
        if ($this->session->userdata('login_but_verify') && !empty($email)) {
            ...
            if (!empty($pwd_details)) {
                ...
                $email_data['subject'] = $this->config->item('project_name').'Registration - Verify your Account with 24 hours';
                $email_data['email_header'] = $this->config->item('project_name').'Registration - Account Verification';
                $email_data['name'] = ucfirst($pwd_details[0]->name);
