...
//    }
      ...
      //  print_r($post);die;
       ...
       // echo '<pre>'; print_r($data); echo '</pre>';die;
        ...
        if (!$data['error']) {
            ...
            $endTS = strtotime(gmdate('Y-m-d 23:59:59')); 
            $params['from'] = $startTS;
            $params['to'] = $endTS;
            ...
            $var->xaxissize = $size = sizeof($answerData2['csv']);
                ...
                //$var->profit_loss=$balance-$deposit-$withdrawal;
                $var->profit = $deposit + $profit_loss + $withdrawal;
    ...
    public function depositnew_form($page_id = 0) {
		...
		if(isset($page_id) && ($page_id=='bwt2' || $page_id=='bwt3') ){
		$this->load->view('userpages/depositnew_form_invoice',$data);
		}else{
   ...
   $is_pdf=true;	 
	 if(empty($id) && isset($_POST) && count($_POST)>0)
	  ...
	  if($is_pdf)
	  {
		...
		if($data['bank']=='localexchanger')
		{
		  $pdf_data=$this->load->view('userpages/depositnew_form_invoice_download_exchanger', $data, true);
		}
		else{
		$pdf_data=$this->load->view('userpages/depositnew_form_invoice_download', $data, true);
		}
	    $this->html2pdf->html($pdf_data);
	  ...
	  if($data['bank']=='localexchanger')
		{
	   $this->load->view('userpages/depositnew_form_invoice_download_exchanger',$data);
	   }else{
    ...
    public function my_invoices() {
		error_reporting(E_ALL);ini_set('display_errors','on');
        $this->load->view('userpages/my_invoicelist');
    }
    ...
    public function get_invoicelist()
	{
		error_reporting(E_ALL);ini_set('display_errors','on');
		$userDetails = unserialize($this->session->userdata['fx_user_details']);  
        echo $this->withdrawal_model->get_myinvoicelist($userDetails->id);
    }
    ...
    public function trading_history($msg='') {
        ...
        if (!$data['error']) {
            ...
            $endTS = strtotime(date($to.' 23:59:59' )); 
            $params['from'] = $startTS;
            $params['to'] = $endTS;
            $var->answerData2=$answerData2 = $mt4request->MakeRequest("gethistory", $params);   
...
*/
    ...
    public function remove_pamm()
    {
		...
		if($data->id!='' && $data->id!='0')
		{
			...
			$email_data['from'] = $this->config->item('from_mail');
			$email_data['to'] = $this->db->escape_str($user_details->email);
			$email_data['subject'] = 'Your request to remove the PAMM Program';
			$email_data['email_header'] = 'ForexRay - PAMM Program Removal';
			$email_data['name'] = ucfirst($user_details->firstname);
			$email_data['content'] = $this->load->view('email_templates/template_2/after_joinpamm', $email_data, true);
			$resultuser = $this->mail_model->send_mail($email_data);
    ...
    public function save_depositmt4() {
           if (formtoken::validateToken($_POST)){
            ...
            if($this->input->post('page')=='depositmt4'){
				...
				//email content
				$email_data['login_id'] = $userwallet->login;
				//$email_data['password'] = '2342';
				//$email_data['registration_type'] = '44';
				//$email_data['phone_password'] = $post['phone_password'];
				$email_data['from'] = $this->config->item('from_mail');
				$email_data['to'] = $this->db->escape_str($user_details->email);
				$email_data['subject'] = 'ForexRay - ' . ucfirst($user_details->firstname) . ', Your Funds Added Successfully';
				$email_data['email_header'] = 'ForexRay - Your Funds Added Successfully.';
				$email_data['name'] = ucfirst($user_details->firstname);
				$email_data['message'] = 'Your Funds have been added to the PAMM Program successfully.';
    ...
    public function internal_transfer($msg=''){
        ...
        {
                $message='Your request has been submitted successfully. We will process your request';
        }
        else $message='';
        $data->msg=$msg;
        $data->message=$message;
    ...
    public function save_it()
    {
            if (formtoken::validateToken($_POST)){
				...
				$post=$this->input->post();
				$sql="insert into internal_transfer (from_account,to_account,amount) values ('".$user_details->login."','".$post['acount_no']."','".$post['amount']."')";
				$this->db->query($sql);
				redirect('userpages/internal_transfer/5');
				/*
                $post=$this->input->post();
                $userwallet=$this->users_model->get_wallet($user_details->id);
                $wallet=$userwallet->wallet;
    ...
    public function investor_history($investorid,$msg='') {
        //echo $investorid;
        $invid=base64_decode($investorid);
        //exit();
        $var = new stdclass();
        $var = $this->users_model->userdata($invid);
		...
		$var->investorid=$investorid;echo '<br/>';
		//echo base64_decode($in1);echo '<br/>';
        if (!$data->error) {
            $time=mktime(23,59,59,date('n'),date('j'),date('Y'));
            $time2=$time-2592000;
            $from=($this->input->post('from')=='')?gmdate("Y-m-d", $time2):$this->input->post('from');
            ...
            $this->load->view('userpages/investor_history', $var);
        } else {
            redirect('login/logout');
        }
    }
            ...
            for($i = 0; $i < $size; $i++)
            {
               ...
               // if($values[12]=='1' || $values[12]=='0')
                   // {
                            $profit_loss=$profit_loss+($values[5])+($values[15]);echo '<br/>';
                    //}
                            echo gmdate('Y-m-d 23:59:59');echo '<br/>';
                            echo date('Y-m-d 23:59:59');
                echo '<pre>';
                print_r($values);
                echo $profit_loss;echo '<br/>';
                echo $values15+=$values[15];echo '<br/>';
                echo $values5+=$values[5];echo '<br/>';
            }
            echo $var->profit_loss=$profit_loss;
            exit();
    ...
    {
        $data='';
        $this->load->view('userpages/payementgateway',$data);
    }

    public function curl_payment()
    {
        $data='';
        $this->load->view('userpages/curl_payment',$data);
    }

    public function test_profile($msg=''){
		$data = '';
        $user_details = unserialize($this->session->userdata['user_details']);
        $data=$this->users_model->mydata($user_details->id);
        ...
        $data->msg=$msg;
        $data->message=$message;
        $this->load->view('userpages/test_profile',$data);
    }
