...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$(this).addClass('ui-jqgrid-btable').addClass('scroll').append(firstr);
		firstr = null;
		var hTable = $("<table class='ui-jqgrid-htable scroll' style='width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(thead),
        ...
        if(ts.p.pager){
			...
			// $(ts.p.pager).css({width: grid.width+"px"}).appendTo(eg).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom');
                        $(ts.p.pager).css({width: grid.width+"px"}).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom');  // Removed .appendTo(eg) to prevent the pager from attaching to grid.
		...
		$(grid.bDiv)
			.append($('<div class="ie_position_adjust" style="position:relative;'+(isMSIE && $.browser.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
